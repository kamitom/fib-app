#!/bin/bash
# Wrapper script for AWS infrastructure scripts with logging
# Usage: ./setup-aws-infrastructure-with-logging.sh
#
# This wrapper:
...
RAW_LOG="$LOG_DIR/setup-${TIMESTAMP}-raw.log"
FINAL_LOG="$LOG_DIR/setup-${TIMESTAMP}.log"

echo "================================================"
echo "AWS 基礎設施建立 (帶日誌記錄)"
echo "================================================"
echo ""
...
{
  "$SCRIPT_DIR/setup-aws-infrastructure.sh"
} 2>&1 | tee "$RAW_LOG"
...
echo ""
echo "清理舊日誌..."
ls -t "$LOG_DIR"/setup-*.log 2>/dev/null | tail -n +11 | xargs -r rm || true
echo "✓ 保留最近 10 個日誌"
